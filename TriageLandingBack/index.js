...
app.get("/", (req, res) => {
  ...
  app.get('/api/v1/admin', authenticateToken, async (req, res, next) => {
    ...
    res.status(200).json({message: "aqui esta tu contenido privado"});
  ...
  try {
    ...
    res.status(200).json({data:MockUser, message: "Login correcto", token:token});
